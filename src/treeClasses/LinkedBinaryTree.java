...
public class LinkedBinaryTree<E> extends AbstractBinaryTree<E> implements Cloneable {
	...
	// Creating a CLONE
		public LinkedBinaryTree<E> clone() throws CloneNotSupportedException { 
			...
			if (!isEmpty())
				t.addRoot(root().getElement()); 
			cloneSubtree(root(), t, t.root()); 
			
			return t; 
		}

		private void cloneSubtree(Position<E> rThis, LinkedBinaryTree<E> other, Position<E> rOther) {
			for (Position<E> pOfT : children(rThis)) {
				Position<E> parent = this.parent(pOfT);
				Position<E> pOther = null;
				if(this.hasLeft(parent) && this.left(parent).equals(pOfT))
					pOther = other.addLeft(rOther, pOfT.getElement());
				else
					pOther = other.addRight(rOther, pOfT.getElement());
				cloneSubtree(pOfT, other, pOther); 
			}
		}
